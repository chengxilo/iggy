...
func NewIggyMessage(payload []byte, opts ...IggyMessageOpt) (IggyMessage, error) {
	if len(payload) == 0 {
		return IggyMessage{}, ierror.New(ierror.InvalidMessagePayloadLength)
	}

	if len(payload) > MaxPayloadSize {
		return IggyMessage{}, ierror.New(ierror.TooBigMessagePayload)
	}
	...
	if userHeaderLength > MaxUserHeadersSize {
		return IggyMessage{}, ierror.New(ierror.TooBigUserHeaders)
	}
